...
\author{%
  Nikhil Katre\thanks{Equal contribution.}\\
  \textit{Independent Researcher}\\
  \texttt{npkatre@gmail.com}
  \and
  Shweta Mane\footnotemark[1]\\
  \textit{Independent Researcher}\\
  \texttt{maneshweta1991@gmail.com}%
}

\date{March 2026 \quad \texttt{arXiv:XXXX.XXXXX [cs.SE]}}
...
We evaluate CARA on a 15-case benchmark spanning five change categories. CARA
achieves \textbf{0.87 F1} on breaking-change detection, \textbf{0.91}
expand-contract compliance, and \textbf{0.83} plan completeness, outperforming
both rule-based and prompt-only baselines by substantial margins. All code,
...
\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
  ...
  \item \textbf{Open benchmark.} 15 annotated refactoring scenarios across five
  categories, including multi-account patterns absent from prior benchmarks.
  \item \textbf{Empirical evaluation.} CARA achieves 0.87 F1 and 0.80 overall
  pass rate, substantially outperforming both baselines.
\end{enumerate}
...
reasoning loop~\cite{yao2023react} drives a Claude claude-sonnet-4-6 LLM with native tool
...
Our benchmark contains 15 refactoring scenarios across five categories:

\begin{itemize}[leftmargin=*,itemsep=1pt,topsep=2pt]
  \item \textbf{Schema-only} (3): field addition, type widening, column removal.
  \item \textbf{API-only} (3): endpoint path change, response addition, request field removal.
  \item \textbf{Combined schema + API} (4): coordinated renames, table splits, type cascades.
  \item \textbf{Multi-account} (3): per-account-type impact, shared model splits.
  \item \textbf{Complex/edge} (2): circular cascades, phased multi-service migrations.
\end{itemize}
...
severity labels, plan requirements, and adapter necessity. All 15 cases and
evaluation scripts are released with the paper.
...
\textbf{LLM-only:} Claude claude-sonnet-4-6 with no tools, prompted with the same
...
\begin{table}[h]
\centering
\caption{CARA evaluation results on the 15-case benchmark.}
\label{tab:results}
\small
...
\midrule
Breaking Change Precision & \textbf{0.89} & 0.75 & 0.61 \\
Breaking Change Recall    & \textbf{0.85} & 0.80 & 0.57 \\
Breaking Change F1        & \textbf{0.87} & 0.77 & 0.59 \\
\midrule
Plan Completeness         & \textbf{0.83} & N/A  & 0.41 \\
Expand-Contract Compliance & \textbf{0.91} & N/A & 0.33 \\
Rollback Presence Rate    & \textbf{0.93} & N/A  & 0.27 \\
\midrule
Adapter Generation Rate   & \textbf{0.82} & N/A  & 0.00 \\
Overall Pass Rate         & \textbf{0.80} & 0.47 & 0.20 \\
\midrule
Mean Latency (s)          & 18.4          & 0.1  & 2.1  \\
\bottomrule
\end{tabular}
\end{table}
...
\addplot[fill=darkgray!80, draw=darkgray] coordinates {
  (F1,0.87) (Completeness,0.83) (Expand-Ctr,0.91) (Pass Rate,0.80)};
...
\paragraph{LLM non-determinism} Results reflect single runs per case.
A larger study averaging multiple runs at temperature 0 would yield more
stable estimates.
...
in four languages. On our 15-case benchmark, CARA achieves 0.87 F1 and
0.80 overall pass rate, outperforming both rule-based and prompt-only baselines
by wide margins.
...
\url{https://github.com/nkatre/cara-agent}
