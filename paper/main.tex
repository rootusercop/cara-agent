...
\tikzset{
  box/.style={rectangle, rounded corners=4pt, draw=darkgray, thick,
              fill=white, text centered, align=center, minimum height=1.8em, minimum width=4.5em,
              font=\small},
  ...
  phase/.style={rectangle, rounded corners=4pt, draw=darkgray, thick,
                fill=white, text centered, align=center, minimum height=2em, minimum width=5em,
                font=\small\bfseries},
...
We evaluate the CARA tool pipeline on a 515-case benchmark (15 expert-crafted
plus 500 programmatically generated from real-world patterns) spanning five
change categories. CARA achieves \textbf{0.86 F1} on breaking-change
detection, \textbf{1.00} expand-contract compliance, and \textbf{0.97} plan
completeness, outperforming both rule-based and prompt-only baselines by
substantial margins. The tool pipeline is deterministic: five repeated runs
yield standard deviation 0.000 across all metrics. All code, benchmark data,
...
We are unaware of a system that unifies both layers into a single reasoning agent.
...
We evaluate the CARA \emph{tool pipeline} directly, invoking all seven tools in
their canonical sequence without LLM mediation. This approach serves two
purposes. First, it isolates the quality of the analytical tools from LLM
sampling variance, yielding fully reproducible results (std = 0.000 across 5
repeated runs). Second, it enables evaluation at the 515-case scale without
prohibitive API cost.

The tool pipeline is the core contribution of this work: every detection,
classification, plan step, and adapter is produced by the seven tools
(Section~4). The ReAct LLM agent's role is to select and sequence these tools
for open-ended natural-language requests; for the structured cases in our
benchmark the canonical sequence is fixed. Consistent with this, running the
tool pipeline on the 15 expert-crafted cases produces a 0.80 pass rate and
0.96 mean score, identical to the originally reported LLM-agent result on the
same cases, confirming that the agent correctly delegates to the tool layer.

\subsection{Results}

Table~\ref{tab:results} presents quantitative results for the CARA tool
pipeline. The Rule-only and LLM-only baselines are evaluated on the 15
expert-crafted cases. Figure~\ref{fig:results} visualizes the comparison.

\begin{table}[h]
\centering
\caption{CARA tool pipeline on 515 cases (5-run mean; std = 0.000). Baselines evaluated on 15 expert cases.}
\label{tab:results}
\small
...
\paragraph{Expert cases: schema-only (01--03)} The following qualitative analysis
refers to the 15 expert-crafted cases run through the full tool pipeline.
CARA correctly classifies all severities. The INT-to-BIGINT widening is labeled
BACKWARD\_COMPATIBLE with an ORM update warning, matching ground truth exactly.
The LLM-only baseline (no tools) misclassifies it as BREAKING in 2 of 3 runs.

\paragraph{Expert cases: API-only (04--06)} CARA detects the endpoint path change
as BREAKING and generates a three-phase expand-contract plan with a deprecation
notice step. The LLM-only baseline omits the rollback checkpoint in 4 of 5 runs.

\paragraph{Expert cases: combined (07--10)} Case 07 shows CARA's clearest advantage.
By processing both layers in a single pipeline pass, it recognizes that
\texttt{full\_name} removal appears in \emph{both} the schema and API response
and generates one coordinated plan. The rule-only baseline produces two
independent plans that create an inconsistent intermediate state.

\paragraph{Expert cases: multi-account (11--13)} Case 11 demonstrates per-consumer-group
...
that service but SAFE for all others. The rule-only baseline labels it globally BREAKING.

\paragraph{Expert cases: complex (14--15)} The cascading type change (Case 14)
requires all seven tools across both layers. CARA passes with score 0.78. The
phased migration (Case 15) produces a matching 3-phase plan but misses one
consumer reference due to string-interpolated SQL, yielding a warning.

\subsection{Error Analysis}

Across the 515-case benchmark, 64 cases fail (pass rate 0.876). The complex
category accounts for the majority of failures: 40 of 53 complex cases fail,
reflecting the inherent difficulty of multi-step cascading migrations. The
remaining 24 failures are distributed across the schema-only (16) and combined
(4) and multi-account (4) categories. Three primary failure modes emerge:

\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
  \item \textbf{Insufficient expand steps} (16 cases): The plan generator
  produces fewer steps than required by ground truth for NOT NULL column
  additions and column removals. The pipeline correctly identifies the
  change as BREAKING but underestimates the migration complexity.
  \item \textbf{Complex multi-field cascades} (40 cases): Scenarios
  requiring simultaneous rename of 3 or more fields across both layers
  exceed the plan generator's current step-count heuristics.
  \item \textbf{Consumer undercounting} (8 cases): The regex tracer misses
...
severity. Parnin et al.~\cite{parnin2022dozer} demonstrated automated migration
synthesis in the infrastructure-as-code domain, migrating imperative shell
commands to declarative Ansible modules via execution profiling. CARA applies
analogous synthesis to the schema-and-API domain, where the migration target
is a coordinated expand-contract plan rather than a single-layer command translation.
...
LLM-based automated repair. Parnin et al.~\cite{parnin2025copilot} identified
key challenges in shipping LLM-powered copilots inside software products,
noting that tool grounding and context scoping are the primary failure modes.
None of these address distributed system refactoring or the dual-layer
coordination problem. The ReAct framework~\cite{yao2023react} motivates our
agent's reasoning structure; chain-of-thought prompting~\cite{wei2022chainofthought}
...
\paragraph{Internal validity} Fifteen benchmark cases are expert-crafted from
real production incidents; the remaining 500 are programmatically generated
from parameterized templates. Template-generated cases have limited variance
within each pattern family. They may not capture the full diversity of
production refactoring tasks. We partially mitigate this with complex and
multi-account categories absent from prior benchmarks.
...
\paragraph{Evaluation pipeline} The 515-case benchmark evaluates the CARA
tool pipeline without LLM inference. The full ReAct LLM agent is not
evaluated at this scale; doing so would require approximately 2.5 hours of
API calls. On the 15 expert-crafted cases, the tool pipeline and LLM agent
produce identical results (0.80 pass rate), supporting the claim that the
tool layer captures the system's analytical quality. Large-scale LLM-agent
evaluation is left as future work.

\paragraph{Reproducibility} Because the evaluation uses no LLM sampling, all
results are fully deterministic. Five repeated runs yield standard deviation
0.000 on every metric.
